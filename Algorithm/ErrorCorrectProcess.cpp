...
ErrorCorrectResult ErrorCorrectProcess::correct(const SequenceWorkItem& workItem)
{
    switch(m_params.algorithm)
    {
		...
		case ECA_PACOVERLAP:
		{
			return PacBioOverlapCorrection(workItem);
			break;
		}
        default:
        {
...
ErrorCorrectResult ErrorCorrectProcess::PacBioOverlapCorrection(const SequenceWorkItem& workItem)
{
	//std::cout<<"In PacBioOverlapCorrection\n";
	...
	//Construct frequence array
	SequenceFrequence query_frequence(current_sequence, m_params);

    int round = 0;
	
	Timer total("total", true);
	
	while(1)
	{	
		// Construct the multiple alignment
		//If need to decrease kmer, but DON'T update frequence array		
		/*while(query_frequence.isKmerNeedDecrease(m_params))
			query_frequence.decreaseKmer();*/
		//std::cout<<"\tcontinueInterval : "<<query_frequence.continueInterval()<<std::endl;
		
		
		MultipleAlignment multiple_alignment = KmerOverlaps::PacBioBuildMultipleAlignment(current_sequence,
																					query_frequence.getkmerLength(),
																					m_params.minOverlap,
		...
		Timer correct("correct", true);
		current_sequence = multiple_alignment.calculateBaseConsensus(10000, 0);
		printf("\tcorrect time : %lf\n", correct.getElapsedWallTime());
		//If completely correct and update frequence array
		//Check round times
		bool last_round = (query_frequence.isCompletelyCorrect(current_sequence, m_params)
						|| (round + 1 == m_params.numOverlapRounds));
		if(last_round)
		{
			consensus = current_sequence;
			std::cout<<"\tquery length : "<<consensus.size()<<std::endl;
			//query_frequence.isKmerNeedIncrease(round);
			break;
		}
		else
		{
			//If need to increase kmer by continue interval, and update frequence array
			query_frequence.isKmerNeedIncrease(round);
		...
		if(m_params.printOverlaps)
			query_frequence.outputKmerFrequence(m_params);
