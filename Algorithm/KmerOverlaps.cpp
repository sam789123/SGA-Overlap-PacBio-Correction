...
MultipleAlignment KmerOverlaps::PacBioBuildMultipleAlignment(const std::string& query, 
                                                       size_t k,
                                                       int min_overlap,
                                                       double min_identity,
                                                       int bandwidth,
													   ...
													   int round)
{
    SequenceOverlapPairVector overlap_vector = PacBioRetrieveMatches(query, k, min_overlap, min_identity, bandwidth, indices, kd, round);
    MultipleAlignment multiple_alignment;
    multiple_alignment.addBaseSequence("query", query, "");
	clock_t addOverlap_s, addOverlap_e;
	addOverlap_s = clock();
    for(size_t i = 0; i < overlap_vector.size(); ++i)
        multiple_alignment.addOverlap("null", overlap_vector[i].sequence[1], "", overlap_vector[i].overlap);
	addOverlap_e = clock();
	std::cout<<"\taddOverlap time : "<< (double)(addOverlap_e - addOverlap_s)/CLOCKS_PER_SEC<<std::endl;
    return multiple_alignment;
}
