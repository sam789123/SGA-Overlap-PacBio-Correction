...
"      -T, --readtable=readtable        read pair end read from readtable\n"
...
static const char* shortopts = "p:T:m:d:e:t:l:s:o:r:b:a:c:k:x:i:v";
...
static const struct option longopts[] = {
	...
	{ "readtable",     required_argument, NULL, 'T' },
...
int correctMain(int argc, char** argv)
{
	...
	//std::cout<<"opt::algorithm == "<<opt::algorithm;
	
	std::cout <<"-------------------------------------"<<std::endl;
	std::cout <<"\tprefix:"<<opt::prefix<<std::endl;
	std::cout <<"\treadtable:"<<opt::readtable<<std::endl;
	std::cout <<"\tthread:"<<opt::numThreads<<std::endl;
	std::cout <<"\talgorithm:"<<opt::algorithm<<std::endl;
	std::cout <<"\terror-rate:"<<opt::errorRate<<std::endl;
	std::cout <<"\tidentity:"<<1.0f - opt::errorRate<<std::endl;
	std::cout <<"\tmin-overlap:"<<opt::minOverlap<<std::endl;
	std::cout <<"\tround:"<<opt::numOverlapRounds<<std::endl;
	std::cout <<"-------------------------------------"<<std::endl;
	
    std::cout << "Correcting sequencing errors for " << opt::readsFile <<std::endl;
	std::cout << "Loading "<<opt::prefix<<" index for "<< opt::readsFile <<std::endl;
    // Load indices
    BWT* pBWT = new BWT(opt::prefix + BWT_EXT, opt::sampleRate);
    BWT* pRBWT = NULL;
    SampledSuffixArray* pSSA = NULL;

    if(opt::algorithm == ECA_OVERLAP || opt::algorithm == ECA_HYBRID || opt::algorithm == ECA_PACOVERLAP)
	...
	//create read table
	clock_t table_s,table_e;
    table_s = clock();
	ReadTable reads(opt::prefix + ".fasta", 2);
	table_e = clock();
    //std::cout<<"\tcreate talbe time : "<<(double)(table_e - table_s)/CLOCKS_PER_SEC<<std::endl;
	//else
	...
	indexSet.pReadTable = &reads;
	//std::cout<<"number of read table : "<<indexSet.pReadTable->getCount()<<std::endl;
	...
	std::vector<std::ostream*> writer;
	for(size_t i =0; i < opt::numThreads; i++)
	{
		std::stringstream ss;
		ss<<i;
		std::ostream *outfiles = createWriter(opt::outFile + "-thread" + ss.str());
		writer.push_back(outfiles);
	}
	...
	ecParams.kd = BWTAlgorithms::sampleKmerCounts(opt::minOverlap, 100000, pBWT);
	...
	std::vector<ErrorCorrectPostProcess*> postProcessorVector;
	for(size_t i = 0; i < opt::numThreads; i++)
	{
		ErrorCorrectPostProcess *postProcess = new ErrorCorrectPostProcess(writer[i], pDiscardWriter, bCollectMetrics);
		postProcessorVector.push_back(postProcess);
	}
    ...
    {
		std::cout<<"In Parallel mode\n";
        // Parallel mode
		
        std::vector<ErrorCorrectProcess*> processorVector;
        //std::vector<ErrorCorrectPostProcess*> postProcessorVecor;
		...
		std::vector<std::string> threadsFile;
		
		for( size_t i = 0; i < opt::numThreads; i++)
		{
			std::stringstream ss;
			ss<<i;
			threadsFile.push_back(opt::readsFile + "-thread" + ss.str());
			std::cout<<"file : "<<opt::readsFile + "-thread" + ss.str()<<std::endl;
		}
		
		/*for(int i = 0; i < opt::numThreads; ++i)
        {
            ErrorCorrectPostProcess* postProcessor = new ErrorCorrectPostProcess(pWriter[i], pDiscardWriter, bCollectMetrics);
            postProcessorVecor.push_back(postProcessor);
        }*/
		
		#pragma omp parallel for
		for(size_t i = 0; i < opt::numThreads; i++)
			SequenceProcessFramework::processSequencesParallelOpenMPbyYi_De<SequenceWorkItem,
                                                           ErrorCorrectResult, 
                                                           ErrorCorrectProcess, 
                                                           ErrorCorrectPostProcess>(threadsFile[i], processorVector[i], postProcessorVector[i]);
		
		
		/*SequenceProcessFramework::processSequencesParallelOpenMPbyYi_De<SequenceWorkItem,
                                                           ErrorCorrectResult, 
                                                           ErrorCorrectProcess, 
                                                           ErrorCorrectPostProcess>(threadsFile, processorVector, postProcessorVector);*/
														   
        /*for(int i = 0; i < opt::numThreads; ++i)
        {
            delete processorVector[i];
			delete postProcessorVector[i];
        }*/	
		
    }
	
	for(size_t i = 0; i < 1/*opt::numThreads*/; i++)
		if(bCollectMetrics)
		{
			std::ostream* pMetricsWriter = createWriter(opt::metricsFile);
			postProcessorVector[i]->writeMetrics(pMetricsWriter);
...
void parseCorrectOptions(int argc, char** argv)
{
    ...
    for (char c; (c = getopt_long(argc, argv, shortopts, longopts, NULL)) != -1;) 
    {
        ...
        switch (c) 
        {
			...
			case 'T': arg >> opt::readtable; break;
    ...
    if(!algo_str.empty())
    {
		...
		else if(algo_str == "Pacoverlap")
			opt::algorithm = ECA_PACOVERLAP;
        else
        {
