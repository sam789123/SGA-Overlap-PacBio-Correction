...
void ThreadWorker<Input, Output, Processor>::run()
{
    ...
    while(1)
    {
		Timer semwait("thread sem_wait", true);
        // Block until there is some data to use
        sem_wait(&m_producedSem);
        printf("in threadwork sem_wait thread  %d wait time : %lf\n", m_pReadySem, semwait.getElapsedWallTime());
        ...
        printf("in threadwork thread %d time : %lf\n", m_pReadySem, process.getElapsedWallTime());
	m_isReady = true;
        pthread_mutex_unlock(&m_mutex);
